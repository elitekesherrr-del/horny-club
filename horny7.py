...
# ==================== VIDEO MAPPING (from working script) ====================
VIDEOS = {
...
def reply_menu():
    ...
    for i in range(32, 41, 2):
        ...
        keyboard.append(row)
    # Admin panel button (visible to all, but requires code)
...
async def handle_admin_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Check the entered code and grant access"""
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    # No user ID check – anyone who reached this menu already entered the code
    if query.data == "admin_users":
        ...
        try:
            # Try to forward message ID 2 (class1)
...
async def verify_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not not_joined:
        await query.message.edit_text("✅ Verification Successful! Now enjoy the videos!")
    else:
...
async def text_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # For video buttons: first check join status
    not_joined = await check_join(context.bot, user_id)
    if not_joined:
        await update.message.reply_text(random.choice(JOIN_MESSAGES), reply_markup=get_join_keyboard(not_joined))
        return

    # Map button texts to class keys
    button_map = {}
    for i in range(1, 32):
    ...
    if text in button_map:
        ...
        try:
            ...
            # Optionally log error (uncomment next line to send to a private log channel)
...
async def set_commands(app):
    await app.bot.set_my_commands([
        ...
        BotCommand("admin", "Open admin panel (code required)")
    ])
...
async def run_bot():
    ...
    app.add_handler(MessageHandler(filters.ALL, handle_broadcast))  # must come before text_buttons
